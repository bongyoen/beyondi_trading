...
void main(List<String> args) async {
  ...
  final isPaper = appKey.startsWith('PS')
      ? true
      : ((config['is_paper'] as bool?) ?? true);
  final baseUrl = isPaper
  ...
  for (int i = 0; i < allStocks.length; i++) {
    ...
    if (existing != null && existing.length > 500) {
      skip++;
      continue;
    }
    ...
    final candles = await _downloadMinute(
      token,
      appKey,
      appSecret,
      baseUrl,
...
Future<String> _getToken(String key, String secret, String baseUrl) async {
  ...
  try {
    ...
    if (j['access_token'] == null) {
      print('토큰 실패: ${j['msg1'] ?? j['error_description'] ?? body}');
      return '';
    }
...
Future<List<Map<String, dynamic>>> _fetchChunk(
  ...
  try {
    ...
    r.headers.set('appkey', key);
    r.headers.set('appsecret', secret);
    r.headers.set('custtype', 'P');
    r.headers.set('tr_id', 'FHKST03010230');
    final res = await r.close();
    final body = await res.transform(utf8.decoder).join();
    final j = jsonDecode(body) as Map<String, dynamic>;
    if (j['rt_cd'] != '0') return [];
    return ((j['output2'] as List<dynamic>?) ?? [])
        .map(
          (e) => {
  ...
  for (int d = 0; d <= end.difference(start).inDays; d++) {
    ...
    final chunks = await Future.wait(
      [
        ...
        '133000',
        '153000',
      ].map((t) => _fetchChunk(token, key, secret, baseUrl, code, date, t)),
    );
    final seen = <String>{};
    ...
    if (d % 30 == 0)
      print('  ${d}/${end.difference(start).inDays}일 (${all.length}캔들)');
...
List<Map<String, dynamic>>? _loadCached(String dir, String symbol) {
  ...
  if (fullFile.existsSync()) {
    return (jsonDecode(fullFile.readAsStringSync()) as List<dynamic>)
  ...
  final data = (jsonDecode(oldFiles.first.readAsStringSync()) as List<dynamic>)
      .cast<Map<String, dynamic>>();
...
void _saveCandles(
  ...
  if (file.existsSync()) {
    try {
      existing = (jsonDecode(file.readAsStringSync()) as List<dynamic>)
          .cast<Map<String, dynamic>>();
    } catch (_) {}
  }
