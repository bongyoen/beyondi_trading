...
void _saveReport(String cacheDir) {
  ...
  // 추천 TOP 10 (300K_20d 기준)
  r += '## 추천 TOP 10 (300K_20d 기준)\n\n';
  r += '| 순위 | 종목 | 순손익 | 수익률 | 승률 | 거래 | MDD | Sharpe |\n';
  r += '|---|---|---|---|---|---|---|---|\n';

  final f20 = File('$cacheDir\\orb_batch_300K_20d.json');
  if (f20.existsSync()) {
    final data = (jsonDecode(f20.readAsStringSync()) as List).cast<Map<String, dynamic>>();
    data.sort((a, b) => (b['netReturn'] as num).compareTo(a['netReturn'] as num));
    final top = data.where((e) => (e['netReturn'] as num) > 0).toList();
    for (int i = 0; i < top.length && i < 10; i++) {
      r += _rowMd(i + 1, top[i]);
    }
    if (top.isEmpty) {
  ...
  File('$cacheDir\\orb_batch_report.md').writeAsStringSync(r);
}
