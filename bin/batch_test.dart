...
void main(List<String> args) async {
  final configFile = File(
    '${Platform.environment['APPDATA'] ?? ''}\\com.example\\beyondi_trading\\kis_config.json',
  );
  ...
  for (int idx = 0; idx < stocks.length; idx++) {
    ...
    for (final cfg in configs) {
      ...
      final r = runBacktest(
        ...
        principal: 300000,
        consecutiveLossLimit: (isScalp || m == 'orb' || m == 'ema_trend')
            ? 100
            : 3,
        ...
        longOnly: true,
        // vwap_scalp params
        scalpRsiThreshold: isScalp
            ? (cfg['rsi'] as num?)?.toDouble() ?? 30
            : 30,
        scalpVolumeMultiplier: isScalp
            ? (cfg['vol'] as num?)?.toDouble() ?? 1.5
            : 1.5,
        scalpEntryTicksMin: isScalp
            ? (cfg['minT'] as num?)?.toDouble() ?? 5
            : 5,
        scalpEntryTicksMax: isScalp
            ? (cfg['maxT'] as num?)?.toDouble() ?? 15
            : 15,
        ...
        adxThreshold: m == 'ema_trend'
            ? (cfg['adx'] as num?)?.toDouble() ?? 25
            : 25,
      );
    ...
    for (final sr in stockResults) {
      ...
      print(
        '    ${sr['label']}: ${sign}${(sr['netReturn'] as double).toStringAsFixed(0)}원 (${sr['trades']}건, ${(sr['winRate'] * 100).toStringAsFixed(1)}%)',
      );
    }
    ...
    allResults.add({
      ...
      'bestLabel': bestLabel,
      'netReturn': best,
      'configs': stockResults,
    });
  }

  // 종합 결과
  print('\n\n========== 배치 테스트 결과 ==========');
  final profitable = allResults
      .where((r) => r['netReturn'] != null && (r['netReturn'] as double) >= 0)
      .toList();
  final losing = allResults
      .where((r) => r['netReturn'] != null && (r['netReturn'] as double) < 0)
      .toList();
  final skipped = allResults.where((r) => r['netReturn'] == null).toList();

  allResults.sort(
    (a, b) => ((b['netReturn'] as double?) ?? -999999).compareTo(
      (a['netReturn'] as double?) ?? -999999,
    ),
  );
  ...
  for (final r in profitable) {
    ...
    print(
      '  ${sign}${(r['netReturn'] as double).toStringAsFixed(0)}원 ${r['code']} ${r['name']} (${r['bestLabel']})',
    );
  }
  ...
  File(
    '$saveDir\\batch_results_longonly.json',
  ).writeAsStringSync(jsonEncode(allResults));
...
List<Candle>? _loadCachedMinute(String symbol) {
  ...
  final oldFiles = d
      ...
      .where(
        (f) =>
            f.path.contains('candle_${symbol}_') &&
            !f.path.contains('_full_') &&
            f.path.endsWith('_1d.json'),
      )
      .toList();
